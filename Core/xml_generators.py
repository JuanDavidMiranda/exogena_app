...
def procesar_dinamico_xml(
    ...
    """
    Procesa dinámicamente una hoja de Excel para generar el XML DIAN.
    Detecta la fila de encabezados, identifica columnas críticas por aliases
    y construye el XML según el formato detectado.
    """

    # ==========================================================
    # 1. LEER LA HOJA Y DETECTAR DINÁMICAMENTE EL HEADER REAL
    # ==========================================================
    ...
    fila_inicio_datos = 18  # fallback por defecto
    ...
    for idx in range(15, min(30, len(df_crudo))):
        ...
        if any(
            "DOCUMENTO" in str(f).upper()
            or "IDENTIFICA" in str(f).upper()
            or "RAZON SOCIAL" in str(f).upper()
            for f in valores_fila
        ):
            fila_inicio_datos = idx + 1
            break

    # Releer el Excel usando la fila detectada como encabezado
    ...
    # ==========================================================
    # 3. TRAER CONFIGURACIÓN NORMATIVA DEL FORMATO
    # ==========================================================
    ...
    # ==========================================================
    # 4. IDENTIFICAR COLUMNAS CRÍTICAS CON ALIASES + FALLBACK
    # ==========================================================
    ...
    # Inicializar encabezado temporal
    ...
    # ==========================================================
    # 6. RECORRER REGISTROS Y ARMAR XML
    # ==========================================================
    for _, fila in df_datos.iterrows():
        # Validación de NIT / identificación
        if col_nit not in fila or pd.isna(fila[col_nit]):
            continue

        nit_raw = str(fila[col_nit]).strip().split('.')[0].split('-')[0]

        # Filtrar filas vacías, subtotales, textos, etc.
        if (
            not nit_raw
            or nit_raw.lower() in ['nan', 'none', '', 'null', 'total', 'totales']
            or not nit_raw.isdigit()
        ):
            continue
        ...
        # ======================================================
        # 6.1 EXTRACCIÓN DE MONTOS SEGÚN FORMATO
        # ======================================================
        if fmt_str == "1001":
            columnas_valores = [
                c for c in columnas
                if any(x in c for x in ['PAGO', 'PNDED', 'IDED', 'INDED', 'RETP', 'RETA'])
            ]
            ...
            col_ret = next((c for c in columnas if ('PRACTICARON' in c or 'RETENCION' in c) and c != col_vcom), None)

            vcom_val = entero_seguro(fila[col_vcom]) if col_vcom else 0
            ...
            col_ingbrt = next(
                (
                    ...
                    or 'INGREC' in c
                    or 'ING_REC' in c
                    or 'VALOR ACUMULADO DE LOS INGRESOS' in c
                ),
                None
            )
            ...
            col_ingdev = next(
                (
                    c for c in columnas
                    if ('DEVOLUCIONES' in c or 'ING_DEV' in c or 'DEV' in c) and c != col_ingbrt
                ),
                None
            )
            ...
            atributos_montos = {
                "ingbrt": str(ingbrt_val),
                "ingdev": str(ingdev_val)
            }
            valores_fila_suma = ingbrt_val + ingdev_val

        else:
            # Para otros formatos, intenta tomar columnas numéricas desde la 6ta en adelante
        ...
        # Si toda la fila está en cero, no se reporta
        ...
        # ======================================================
        # 6.2 CREAR NODO DEL REGISTRO
        # ======================================================
        reg = ET.SubElement(mas, tag_registro)

        # Atributos obligatorios base
        cpt_val = (
            str(fila[col_cpt]).split('.')[0].strip()
            if col_cpt and pd.notna(fila[col_cpt])
            else "5001"
        )
        tdoc_val = (
            str(fila[col_tdoc]).split('.')[0].strip()
            if col_tdoc and pd.notna(fila[col_tdoc])
            else "13"
        )
        ...
        if (
            col_dv
            and pd.notna(fila[col_dv])
            and str(fila[col_dv]).strip() != ""
            and str(fila[col_dv]).strip().lower() != 'nan'
        ):
            reg.set('dv', str(fila[col_dv]).strip().split('.')[0])

        # ======================================================
        # 6.3 NOMBRE / RAZÓN SOCIAL
        # ======================================================
        ...
        if razon_social and razon_social.lower() != 'nan':
            ...
            reg.set(
                'apl1',
                str(fila[col_apl1]).strip()
                if col_apl1 and pd.notna(fila[col_apl1]) and str(fila[col_apl1]).lower() != 'nan'
                else ""
            )
            reg.set(
                'apl2',
                str(fila[col_apl2]).strip()
                if col_apl2 and pd.notna(fila[col_apl2]) and str(fila[col_apl2]).lower() != 'nan'
                else ""
            )
            reg.set(
                'nom1',
                str(fila[col_nom1]).strip()
                if col_nom1 and pd.notna(fila[col_nom1]) and str(fila[col_nom1]).lower() != 'nan'
                else ""
            )
            reg.set(
                'nom2',
                str(fila[col_nom2]).strip()
                if col_nom2 and pd.notna(fila[col_nom2]) and str(fila[col_nom2]).lower() != 'nan'
                else ""
            )

        # ======================================================
        # 6.4 UBICACIÓN POR DEFECTO
        # ======================================================
        ...
        # ======================================================
        # 6.5 INYECTAR ATRIBUTOS DE MONTOS
        # ======================================================
        for k, v in atributos_montos.items():
            reg.set(k, v)

    # ==========================================================
    # 7. ACTUALIZAR CABECERA FINAL
    # ==========================================================
    cab_node = mas.find('Cab')
    if cab_node is not None:
        valor_total = cab_node.find('ValorTotal')
        cant_reg = cab_node.find('CantReg')

        if valor_total is not None:
            valor_total.text = str(int(suma_totales_general))

        if cant_reg is not None:
            cant_reg.text = str(int(conteo_registros))

    if conteo_registros == 0:
        raise ValueError(
            f"La hoja '{pestana_seleccionada}' fue procesada, "
            f"pero todos los terceros sumaban $0 en sus columnas de montos principales."
        )
